...
.text
	...
	addu $t9, $t4, $t6	# add the least significant bits of A*B and C*D
	sltu $t8, $t9, $t4	# check whether it has carry-in bit or not 
				# the reason is if $t9 (the sum of two low 32 bits) is less than either $t4 or $t6, it means there is a carryin bit generated
				# and if it has carryin bit generated, $v1 will be 1 as the carry-in bit, otherwise it will be 0.
	addu $t8, $t8, $t5	# add the carry-in bit with the most significant 32 bits of the A*B
	addu $t8, $t8, $t7	# add the result from the previous step with the most significant bits of C*D
